  ...
  return (
    <Sidebar className="bg-sidebar border-sidebar p-2 items-center justify-center" side="left" variant="sidebar" collapsible="icon">
      <SidebarHeader className="px-4 py-5">
        <Link href="/">
          <Image src="/logo-light.png" alt="School logo" width={180} height={3} className="h-6 cursor-pointer dark:hidden"/>
          <Image src="/logo-dark.png" alt="School logo" width={180} height={3} className="h-6 cursor-pointer dark:block" />
        </Link>
      </SidebarHeader>
      <SidebarContent>
        <SidebarGroup>
            ...
            <SidebarMenuItem key={item.title}>
              <SidebarMenuButton className="px-2 py-6 gap-3" asChild>
                <Link href={item.url} className={`flex items‑center px‑4 py‑2`}>
                  <item.icon strokeWidth={isActive ? 2 : 1.6} className={`icon-size ${
                          isActive
